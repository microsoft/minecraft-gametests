...
GameTest.register("SculkTests", "vein_spread_blocks", (test) => {
  ...
  test.succeedWhen(() => {
    for (var x = 0; x < 5; ++x) {
      for (var z = 0; z < 5; ++z) {
        const isSculk =
          test.getBlock(new BlockLocation(x, 3, z)).id === "minecraft:sculk" ||
          test.getBlock(new BlockLocation(x, 4, z)).id === "minecraft:sculk_vein";
...
GameTest.register("SculkTests", "charge_forced_direction", (test) => {
  ...
  test
    ...
    .thenExecuteAfter(TicksPerSecond * 1, () => {
      const expected = [new BlockLocation(1, 7, 2), new BlockLocation(1, 9, 2)];
      const actual = [
